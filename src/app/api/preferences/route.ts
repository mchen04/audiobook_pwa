...
export const PATCH = withMutation(
  ...
  async ({ request, session, data }) => {
    ...
    const isVersionedPath = new URL(request.url).pathname.endsWith("/api/preferences/v2");
    ...
    if (
      isVersionedPath &&
      ((!currentProtocol && !legacyReplayProtocol) || writeIdHeader === null)
    ) {
    ...
    const result = await db.transaction(async (transaction) => {
      ...
      const [originatingSession] = await transaction
        ...
        .from(authSession)
        .where(and(eq(authSession.id, session.session.id), eq(authSession.userId, session.user.id)))
        .limit(1)
        .for("update");
